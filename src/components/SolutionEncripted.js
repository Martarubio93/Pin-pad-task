...
import smile from "../images/face-smile-solid.svg";
...
const SolutionEncripted = () => {
  ...
  return (
    <div className="solutionContainer">
      ...
      <table className="table">
        <thead>
          <tr>
            {numbers.map((num) => {
              return (
                <th key={num} className="table__th">
                  {num}
                </th>
              );
            })}
        ...
        <tbody>
          <tr>
            {lettersAtoI.map((letter) => {
              return (
                <td key={letter} className="table__td">
                  {letter}
                </td>
              );
            })}
      ...
      <p className="solutionContainer__hint">
        Hint: The first digit is 4 and the last one is 1. Good Luck!
        <img className="solutionContainer__hint--img" src={smile} alt="smile" />
      </p>
      <p className="solutionContainer__hint">
        After three wrong attemps the Pinpad will lock for 30 secs
      </p>
    </div>
  );
};
